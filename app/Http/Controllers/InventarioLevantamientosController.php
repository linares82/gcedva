...
class InventarioLevantamientosController extends Controller
{
		...
		if (isset($datos['csv'])) {
			...
			foreach ($table as $row) {
				fputcsv($handle, array(
					$row->no_inventario, $row->plantel_inventario_id, $row->area, $row->escuela, $row->tipo_inventario, $row->ubicacion,
	...
	public function inicioLevantamientoCsv(Request $request)
	{
		...
		if (isset($datos['existe_si']) and !isset($datos['estado_bueno'])) {
			...
			->where('inventarios.existe_si', 'SI')
			->where('il.fecha', $datos['fecha']);

			$resultado_aux2 = Inventario::select('il.fecha', 'pi.name as plantel', 'inventarios.*')
				->join('inventario_levantamientos as il', 'il.id', 'inventarios.inventario_levantamiento_id')
				->join('plantel_inventarios as pi', 'pi.id', 'inventarios.plantel_inventario_id')
				->where('inventarios.plantel_inventario_id', $datos['plantel']);
			if (isset($datos['area'])) {
				$resultado_aux2->where('inventarios.area', $datos['area']);
			}
			if (isset($datos['estado_bueno'])) {
				$resultado_aux2->where('inventarios.estado_bueno', $datos['estado_bueno']);
			}
			if (isset($datos['existe_si']) and isset($datos['estado_bueno'])) {
				$resultado_aux2->where('inventarios.existe_si', $datos['existe_si']);
			}
			
			$resultado_aux2->whereIn('inventarios.estado_bueno', ['MALO', 'BUENO'])
			->where('inventarios.existe_si', 'NO')
			->where('il.fecha', $datos['fecha'])
			->union($resultado_aux);

			$resultado = $resultado_aux2->get();
...
*/
		...
		foreach ($resultado as $row) {
			fputcsv($handle, array(
				$row->id, $row->plantel_inventario_id, $row->area, $row->escuela, $row->tipo_inventario, $row->ubicacion,
				$row->cantidad, $row->nombre, $row->medida, $row->marca,
