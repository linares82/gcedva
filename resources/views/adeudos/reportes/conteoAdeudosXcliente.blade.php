...
@section('header')
    ...
    <div class="page-header">
        <h3><i class="glyphicon glyphicon-plus"></i> General Ingresos </h3>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            {!! Form::open(array('route' => 'adeudos.conteoAdeudosXClienteR', 'id'=>'frm')) !!}
...
<!--                <div class="form-group col-md-6 @if($errors->has('fecha_f')) has-error @endif">
                ...
                <div class="form-group col-md-6 @if($errors->has('st_clientes')) has-error @endif">
                    <label for="st_clientes-field">Estatus del Cliente:</label>
                    {!! Form::select("st_clientes[]", $st_clientes, null, array("class" => "form-control select_seguridad", "id" => "st_clientes-field", "multiple"=>true)) !!}
                    <div id='loading1' style='display: none'><img src="{{ asset('images/ajax-loader.gif') }}" title="Enviando" /></div> 
                    @if($errors->has("st_clientes"))
                    <span class="help-block">{{ $errors->first("st_clientes") }}</span>
                    @endif
                </div>
