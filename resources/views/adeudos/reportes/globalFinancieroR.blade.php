...
<body>
    ...
    <div class="datagrid">
        <h3>Global plantel del mes en curso</h3>
        <table width="50%">
            <thead>
                <th class="th-jefe">
                ...
                <th class="th-jefe">Baja Administrativa</th>
                <th class="th-jefe">Baja Temporal Por Pago</th>
                <th class="th-jefe">Activos Vigentes</th>
                <th class="th-jefe">Suma</th>
                <th class="th-jefe">Preinscritos</th>
                <th class="th-jefe">Pagados</th>
                <th class="th-jefe">No Pagados</th>
            </thead>
            <tbody>
                @php
                $totales=array();
                $totales['baja_administrativa']=0;
                $totales['baja_temporal_por_pago']=0;
                $totales['activos_vigentes']=0;
                $totales['suma_estatus']=0;
                $totales['preinscritos']=0;
                $totales['cuenta_pagados']=0;
                $totales['cuenta_no_pagados']=0;
                $i=0;
                @endphp
                ...
                <tr>
                    <td class="jefe">
                    <a target="_blank" href="{{route('adeudos.alumnosActivosAdeudosPlantelR', array('plantel'=>$r['plantel_id']))}}">
                    {{$r['plantel']}}
                    </a>
                        
                    </td>
                    <td class="jefe">{{$r['baja_administrativa']}}</td>
                    <td class="jefe">{{$r['baja_temporal_por_pago']}}</td>
                    <td class="jefe">{{$r['activos_vigentes']}}</td>
                    <td class="jefe">{{$r['suma_estatus']}}</td>
                    <td class="jefe">{{$r['preinscritos']}}</td>
                    <td class="jefe">{{$r['cuenta_pagados']}}</td>
                    <td class="jefe">{{$r['cuenta_no_pagados']}}</td>
                    @php
                        $totales['baja_administrativa']=$totales['baja_administrativa']+$r['baja_administrativa'];
                        $totales['baja_temporal_por_pago']=$totales['baja_temporal_por_pago']+$r['baja_temporal_por_pago'];
                        $totales['activos_vigentes']=$totales['activos_vigentes']+$r['activos_vigentes'];
                        $totales['suma_estatus']=$totales['suma_estatus']+$r['suma_estatus'];
                        $totales['preinscritos']=$totales['preinscritos']+$r['preinscritos'];
                        $totales['cuenta_pagados']=$totales['cuenta_pagados']+$r['cuenta_pagados'];
                        $totales['cuenta_no_pagados']=$totales['cuenta_no_pagados']+$r['cuenta_no_pagados'];
                    @endphp
                </tr>
                ...
                <tr>
                    <td>Totales</td>
                    <td>{{number_format($totales['baja_administrativa'],2)}}</td>
                    <td>{{number_format($totales['baja_temporal_por_pago'],2)}}</td>
                    <td>{{number_format($totales['activos_vigentes'],2)}}</td>
                    <td>{{number_format($totales['suma_estatus'],2)}}</td>
                    <td>{{number_format($totales['preinscritos'],2)}}</td>
                    <td>{{number_format($totales['cuenta_pagados'],2)}}</td>
                    <td>{{number_format($totales['cuenta_no_pagados'],2)}}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table width="50%">
            <thead>
                ...
                <th>Baja Administrativa</th>
                <th>Baja Temporal Por Pago</th>
                <th>Activos</th>
                <th>Suma</th>
                <th>No Pagados</th>
                <th>Pagados</th>
            ...
            <tbody>
                ...
                $totales['suma_ba']=0;
                $totales['suma_btp']=0;
                $totales['suma_activos_vigentes']=0;
                $totales['suma']=0;
                $totales['suma_no_pagados']=0;
                $totales['suma_pagados']=0;
                $totales['suma_preinscritos']=0;
                ...
                <tr>
                    ...
                    <td>{{number_format($r['suma_ba'],2)}}</td>
                    <td>{{number_format($r['suma_btp'],2)}}</td>
                    <td>{{number_format($r['suma_activos_vigentes'],2)}}</td>
                    <td>{{number_format($r['suma'],2)}}</td>
                    <td>{{number_format($r['suma_no_pagados'],2)}}</td>
                    <td>{{number_format($r['suma_pagados'],2)}}</td>
                    <td>{{number_format($r['suma_preinscritos'],2)}}</td>
                </tr>
                @php
                $totales['suma_ba']=$totales['suma_ba']+$r['suma_ba'];
                $totales['suma_btp']=$totales['suma_btp']+$r['suma_btp'];
                $totales['suma_activos_vigentes']=$totales['suma_activos_vigentes']+$r['suma_activos_vigentes'];
                $totales['suma']=$totales['suma']+$r['suma'];
                $totales['suma_no_pagados']=$totales['suma_no_pagados']+$r['suma_no_pagados'];
                $totales['suma_pagados']=$totales['suma_pagados']+$r['suma_pagados'];
                $totales['suma_preinscritos']=$totales['suma_preinscritos']+$r['suma_preinscritos'];    
                @endphp
                
                @endforeach
                <tr>
                    <td>Totales</td>
                    <td>{{number_format($totales['suma_ba'],2)}}</td>
                    <td>{{number_format($totales['suma_btp'],2)}}</td>
                    <td>{{number_format($totales['suma_activos_vigentes'],2)}}</td>
                    <td>{{number_format($totales['suma'],2)}}</td>
                    <td>{{number_format($totales['suma_no_pagados'],2)}}</td>
                    <td>{{number_format($totales['suma_pagados'],2)}}</td>
                    <td>{{number_format($totales['suma_preinscritos'],2)}}</td>
                </tr>
            </tbody>
        </table>
        <!--        
    ...
    <div id="dinamica_suma_montos"></div>
            -->
    ...
    <script type="text/javascript">
        ...
        var pivot = new WebDataRocks({
            container: "#dinamica_suma_montos",
            beforetoolbarcreated: customizeToolbar,
            toolbar: true,
            report: {
                dataSource: {
                    data: <?php //echo json_encode($tabla); ?>,
                },
                "slice": {
                    "reportFilters": [{
                        "uniqueName": "razon"
