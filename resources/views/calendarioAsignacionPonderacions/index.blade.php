...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('calendarioAsignacionPonderacions.index') }}">@yield('calendarioAsignacionPonderacionsAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('calendarioAsignacionPonderacionsAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('calendarioAsignacionPonderacionsAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('calendarioAsignacionPonderacionsAppTitle')
            @permission('calendarioAsignacionPonderacions.create')
            <a class="btn btn-success pull-right" href="{{ route('calendarioAsignacionPonderacions.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="CalendarioAsignacionPonderacion_search" id="search" action="{{ route('calendarioAsignacionPonderacions.index') }}" accept-charset="UTF-8" method="get">
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            @if($calendarioAsignacionPonderacions->count())
                <table class="table table-condensed table-striped">
                    <thead>
                        ...
                        <th>@include('CrudDscaffold::getOrderlink', ['column' => 'carga_ponderacion_id', 'title' => 'CARGA_PONDERACION_ID'])</th>
                        <th>@include('CrudDscaffold::getOrderlink', ['column' => 'fec_inicio', 'title' => 'FEC_INICIO'])</th>
                        <th>@include('CrudDscaffold::getOrderlink', ['column' => 'fec_fin', 'title' => 'FEC_FIN'])</th>
                    ...
                    <tbody>
                        @foreach($calendarioAsignacionPonderacions as $calendarioAsignacionPonderacion)
                            <tr>
                                <td><a href="{{ route('calendarioAsignacionPonderacions.show', $calendarioAsignacionPonderacion->id) }}">{{$calendarioAsignacionPonderacion->id}}</a></td>
                                <td>{{$calendarioAsignacionPonderacion->asignacion_academica_id}}</td>
                    <td>{{$calendarioAsignacionPonderacion->carga_ponderacion_id}}</td>
                    <td>{{$calendarioAsignacionPonderacion->fec_inicio}}</td>
                    <td>{{$calendarioAsignacionPonderacion->fec_fin}}</td>
                    <td>{{$calendarioAsignacionPonderacion->usu_alta_id}}</td>
                    <td>{{$calendarioAsignacionPonderacion->usu_mod_id}}</td>
                                <td class="text-right">
                                    @permission('calendarioAsignacionPonderacions.edit')
                                    <a class="btn btn-xs btn-primary" href="{{ route('calendarioAsignacionPonderacions.duplicate', $calendarioAsignacionPonderacion->id) }}"><i class="glyphicon glyphicon-duplicate"></i> Duplicate</a>
                                    @endpermission
                                    @permission('calendarioAsignacionPonderacions.edit')
                                    <a class="btn btn-xs btn-warning" href="{{ route('calendarioAsignacionPonderacions.edit', $calendarioAsignacionPonderacion->id) }}"><i class="glyphicon glyphicon-edit"></i> Editar</a>
                                    @endpermission
                                    @permission('calendarioAsignacionPonderacions.destroy')
                                    {!! Form::model($calendarioAsignacionPonderacion, array('route' => array('calendarioAsignacionPonderacions.destroy', $calendarioAsignacionPonderacion->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $calendarioAsignacionPonderacions->appends(Request::except('page'))->render() !!}
            @else
