...
/* 
    ...
    <body>
        ...
        <div id="printeArea" class='SaltoDePagina'>
                <h3>Registro de Informacion de Docentes</h3>
            
            <table>
                <head>
                    <th>RVOE</th><th>CURP</th><th>Clave de Asignatura</th><th>Inicio de impartición de la asignatura</th><th>T. Movimiento</th>
                    
                </head>
                <body>
                    @foreach($registros as $r)
                    <tr>
                    <td>{{$r->rvoe}}</td><td>{{$r->curp}}</td><td>{{$r->clave}}</td><td>{{date_format(date_create($r->inicio),'d-m-Y')}}</td>
                    <td>A</td>
                    </tr>
                @endforeach    
                </body>
