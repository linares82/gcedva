<div class="box box-default">
    <div class="box-body">
        ...
        <div class="form-group col-md-4 @if ($errors->has('especialidad_id')) has-error @endif">
            ...
            <div id='loading2' style='display: none'><img src="{{ asset('images/ajax-loader.gif') }}" title="Enviando" />
            </div>
        ...
        <div class="form-group col-md-4 @if ($errors->has('duracion_periodo_id')) has-error @endif">
            ...
            {!! Form::select('duracion_periodo_id', $duracion_periodo, null, [
                'class' => 'form-control select_seguridad',
        ...
        @permission('esp-nivel-grado_bnd_activo')
        ...
        @if (isset($grado))
            ...
            <div class="form-group col-md-4">
                ...
                <div id="texto_notificacion">
                    <img src="{{ asset('storage/grados/' . $grado->imagen) }}" alt="Logo" height="42"
                        width="42">
                </div>
            </div>
        @endif
    ...
    <div class="box-body">
        ...
        <div class="form-group col-md-4 @if ($errors->has('sep_carrera_id')) has-error @endif">
            ...
            {!! Form::select('sep_carrera_id', $sep_carreras, null, [
                'class' => 'form-control select_seguridad',
                'id' => 'sep_carrera_id-field',
            ]) !!}
            @if ($errors->has('sep_carrera_id'))
                <span class="help-block">{{ $errors->first('sep_carrera_id') }}</span>
            @endif
        </div>
        <div class="form-group col-md-4 @if ($errors->has('sep_autorizacion_reconocimiento_id')) has-error @endif">
            <label for="sep_autorizacion_reconocimiento_id-field">Sep Autorizacion Reconocimiento</label>
            {!! Form::select('sep_autorizacion_reconocimiento_id', $sep_autorizacion_reconocimientos, null, [
                'class' => 'form-control select_seguridad',
                'id' => 'sep_autorizacion_reconocimiento_id-field',
            ]) !!}
            @if ($errors->has('sep_autorizacion_reconocimiento_id'))
        ...
        <div class="form-group col-md-4 @if($errors->has('bnd_servicio_social')) has-error @endif" style="clear:left;">
            ...
            <span class="help-block">{{ $errors->first("bnd_servicio_social") }}</span>
            @endif
        </div>
        <div class="form-group col-md-4 @if ($errors->has('sep_fundamento_legal_servicio_social_id')) has-error @endif">
            <label for="sep_fundamento_legal_servicio_social_id-field">Sep Fundamento Legal Servicio Social</label>
            {!! Form::select('sep_fundamento_legal_servicio_social_id', $sep_fundamento_legal, null, [
                'class' => 'form-control select_seguridad',
                'id' => 'sep_fundamento_legal_servicio_social_id-field',
            ]) !!}
        ...
        <div class="form-group col-md-4 @if ($errors->has('plan_estudio_id')) has-error @endif">
            <label for="plan_estudio_id-field">Plan Estudios</label>
            {!! Form::select('plan_estudio_id', $plan_estudio, null, [
                'class' => 'form-control select_seguridad',
                'id' => 'plan_estudio_id-field',
            ]) !!}
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            function getPlanEstudiosXPlantel() {
                //var $example = $("#especialidad_id-field").select2();
                var a = $('#frm_grados').serialize();
                $.ajax({
                    url: '{{ route('planEstudios.cmbPlanEstudiosXPlantel') }}',
                    type: 'GET',
                    data: {
                        planEstudio: $('#plan_estudio_id-field').val(),
                    ...
                    success: function(data) {
                        //$example.select2("destroy");
                        $('#plan_estudio_id-field').html('');
                        //$('#especialidad_id-field').empty();
                        $('#plan_estudio_id-field').append($('<option></option>').text('Seleccionar')
                            .val(''));
                        $.each(data, function(i) {
                            //alert(data[i].name);
                            $('#plan_estudio_id-field').append("<option " + data[i].selectec +
                                " value=\"" + data[i].id + "\">" + data[i].name +
                                "<\/option>");
                        });
                        //$example.select2();
