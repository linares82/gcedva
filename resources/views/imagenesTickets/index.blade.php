...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('imagenesTickets.index') }}">@yield('imagenesTicketsAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('imagenesTicketsAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('imagenesTicketsAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('imagenesTicketsAppTitle')
            @permission('imagenesTickets.create')
            <a class="btn btn-success pull-right" href="{{ route('imagenesTickets.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="ImagenesTicket_search" id="search" action="{{ route('imagenesTickets.index') }}" accept-charset="UTF-8" method="get">
            ...
            @if($imagenesTickets->count())
                <table class="table table-condensed table-striped">
                    ...
                    <td>{{$imagenesTicket->usu_mod_id}}</td>
                                <td class="text-right">
                                    @permission('imagenesTickets.edit')
                                    <a class="btn btn-xs btn-primary" href="{{ route('imagenesTickets.duplicate', $imagenesTicket->id) }}"><i class="glyphicon glyphicon-duplicate"></i> Duplicate</a>
                                    @endpermission
                                    @permission('imagenesTickets.edit')
                                    <a class="btn btn-xs btn-warning" href="{{ route('imagenesTickets.edit', $imagenesTicket->id) }}"><i class="glyphicon glyphicon-edit"></i> Editar</a>
                                    @endpermission
                                    @permission('imagenesTickets.destroy')
                                    {!! Form::model($imagenesTicket, array('route' => array('imagenesTickets.destroy', $imagenesTicket->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $imagenesTickets->appends(Request::except('page'))->render() !!}
            @else
