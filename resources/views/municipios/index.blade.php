...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('municipios.index') }}">@yield('municipiosAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('municipiosAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('municipiosAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('municipiosAppTitle')
            @permission('municipios.create')
            <a class="btn btn-success pull-right" href="{{ route('municipios.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="Municipio_search" id="search" action="{{ route('municipios.index') }}" accept-charset="UTF-8" method="get">
                        ...
                        <div class="form-horizontal">
                            ...
                            -->
                            <div class="form-group col-md-4">
                                <label class="col-sm-12 control-label" for="q_estados.name_cont">ESTADO_NAME</label>
                                <div class="col-sm-12">
            ...
            @if($municipios->count())
                <table class="table table-condensed table-striped">
                    <thead>
                        ...
                        <th>@include('CrudDscaffold::getOrderlink', ['column' => 'estados.name', 'title' => 'ESTADO_NAME'])</th>
                    ...
                    <tbody>
                        @foreach($municipios as $municipio)
                            <tr>
                                <td><a href="{{ route('municipios.show', $municipio->id) }}">{{$municipio->id}}</a></td>
                                <td>{{$municipio->name}}</td>
                    <td>{{$municipio->estado->name}}</td>
                    <td>{{$municipio->usu_alta_id}}</td>
                    <td>{{$municipio->usu_mod_id}}</td>
                                <td class="text-right">
                                    ...
                                    @endpermission
                                    @permission('municipios.destroy')
                                    {!! Form::model($municipio, array('route' => array('municipios.destroy', $municipio->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
