...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('periodoExamens.index') }}">@yield('periodoExamensAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('periodoExamensAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('periodoExamensAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('periodoExamensAppTitle')
            @permission('periodoExamens.create')
            <a class="btn btn-success pull-right" href="{{ route('periodoExamens.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="PeriodoExaman_search" id="search" action="{{ route('periodoExamens.index') }}" accept-charset="UTF-8" method="get">
        ...
        <div class="col-md-12">
            @if($periodoExamens->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody>
                        @foreach($periodoExamens as $periodoExaman)
                            <tr>
                                <td><a href="{{ route('periodoExamens.show', $periodoExaman->id) }}">{{$periodoExaman->id}}</a></td>
                                <td>{{$periodoExaman->lectivo_id}}</td>
                    <td>{{$periodoExaman->inicio}}</td>
                    <td>{{$periodoExaman->fin}}</td>
                    <td>{{$periodoExaman->usu_alta_id}}</td>
                    <td>{{$periodoExaman->usu_mod_id}}</td>
                                <td class="text-right">
                                    @permission('periodoExamens.edit')
                                    <a class="btn btn-xs btn-primary" href="{{ route('periodoExamens.duplicate', $periodoExaman->id) }}"><i class="glyphicon glyphicon-duplicate"></i> Duplicate</a>
                                    @endpermission
                                    @permission('periodoExamens.edit')
                                    <a class="btn btn-xs btn-warning" href="{{ route('periodoExamens.edit', $periodoExaman->id) }}"><i class="glyphicon glyphicon-edit"></i> Editar</a>
                                    @endpermission
                                    @permission('periodoExamens.destroy')
                                    {!! Form::model($periodoExaman, array('route' => array('periodoExamens.destroy', $periodoExaman->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $periodoExamens->appends(Request::except('page'))->render() !!}
            @else
