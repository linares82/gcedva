...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('seguimientos.index') }}">@yield('seguimientosAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('seguimientosAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('seguimientosAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('seguimientosAppTitle')
            @permission('seguimientos.create')
            <a class="btn btn-success pull-right" href="{{ route('seguimientos.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="Seguimiento_search" id="search" action="{{ route('seguimientos.index') }}" accept-charset="UTF-8" method="get">
    ...
    <div class="row">
        <div class="col-md-12">
            @if($seguimientos->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody>
                        @foreach($seguimientos as $seguimiento)
                            <tr>
                                <td><a href="{{ route('seguimientos.show', $seguimiento->id) }}">{{$seguimiento->id}}</a></td>
                                <td>{{$seguimiento->cliente_id}}</td>
                    <td>{{$seguimiento->usu_alta_id}}</td>
                    <td>{{$seguimiento->usu_mod_id}}</td>
                                <td class="text-right">
                                    ...
                                    @endpermission
                                    @permission('seguimientos.destroy')
                                    {!! Form::model($seguimiento, array('route' => array('seguimientos.destroy', $seguimiento->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $seguimientos->appends(Request::except('page'))->render() !!}
            @else
