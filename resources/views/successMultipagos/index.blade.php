...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('successMultipagos.index') }}">@yield('successMultipagosAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('successMultipagosAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('successMultipagosAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('successMultipagosAppTitle')
            @permission('successMultipagos.create')
            <a class="btn btn-success pull-right" href="{{ route('successMultipagos.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="SuccessMultipago_search" id="search" action="{{ route('successMultipagos.index') }}" accept-charset="UTF-8" method="get">
        ...
        <div class="col-md-12">
            @if($successMultipagos->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody>
                        @foreach($successMultipagos as $successMultipago)
                            <tr>
                                <td><a href="{{ route('successMultipagos.show', $successMultipago->id) }}">{{$successMultipago->id}}</a></td>
                                <td>{{$successMultipago->mp_order}}</td>
                    <td>{{$successMultipago->mp_reference}}</td>
                    <td>{{$successMultipago->mp_amount}}</td>
                    <td>{{$successMultipago->mp_response}}</td>
                    <td>{{$successMultipago->mp_responsemsg}}</td>
                    <td>{{$successMultipago->mp_authorization}}</td>
                    <td>{{$successMultipago->mp_signature}}</td>
                    <td>{{$successMultipago-> usu_alta_id}}</td>
                    <td>{{$successMultipago->usu_mod_id}}</td>
                                <td class="text-right">
                                    @permission('successMultipagos.edit')
                                    <a class="btn btn-xs btn-primary" href="{{ route('successMultipagos.duplicate', $successMultipago->id) }}"><i class="glyphicon glyphicon-duplicate"></i> Duplicate</a>
                                    @endpermission
                                    @permission('successMultipagos.edit')
                                    <a class="btn btn-xs btn-warning" href="{{ route('successMultipagos.edit', $successMultipago->id) }}"><i class="glyphicon glyphicon-edit"></i> Editar</a>
                                    @endpermission
                                    @permission('successMultipagos.destroy')
                                    {!! Form::model($successMultipago, array('route' => array('successMultipagos.destroy', $successMultipago->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $successMultipagos->appends(Request::except('page'))->render() !!}
            @else
