...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('tpoCorreos.index') }}">@yield('tpoCorreosAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('tpoCorreosAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('tpoCorreosAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('tpoCorreosAppTitle')
            @permission('tpoCorreos.create')
            <a class="btn btn-success pull-right" href="{{ route('tpoCorreos.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="TpoCorreo_search" id="search" action="{{ route('tpoCorreos.index') }}" accept-charset="UTF-8" method="get">
        ...
        <div class="col-md-12">
            @if($tpoCorreos->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody>
                        @foreach($tpoCorreos as $tpoCorreo)
                            <tr>
                                <td><a href="{{ route('tpoCorreos.show', $tpoCorreo->id) }}">{{$tpoCorreo->id}}</a></td>
                                <td>{{$tpoCorreo->name}}</td>
                    <td>{{$tpoCorreo->usu_alta_id}}</td>
                    <td>{{$tpoCorreo->usu_mod_id}}</td>
                                <td class="text-right">
                                    ...
                                    @endpermission
                                    @permission('tpoCorreos.destroy')
                                    {!! Form::model($tpoCorreo, array('route' => array('tpoCorreos.destroy', $tpoCorreo->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $tpoCorreos->appends(Request::except('page'))->render() !!}
            @else
