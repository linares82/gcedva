...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('tpoPlantels.index') }}">@yield('tpoPlantelsAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('tpoPlantelsAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('tpoPlantelsAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('tpoPlantelsAppTitle')
            @permission('tpoPlantels.create')
            <a class="btn btn-success pull-right" href="{{ route('tpoPlantels.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="TpoPlantel_search" id="search" action="{{ route('tpoPlantels.index') }}" accept-charset="UTF-8" method="get">
        ...
        <div class="col-md-12">
            @if($tpoPlantels->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody>
                        @foreach($tpoPlantels as $tpoPlantel)
                            <tr>
                                <td><a href="{{ route('tpoPlantels.show', $tpoPlantel->id) }}">{{$tpoPlantel->id}}</a></td>
                                <td>{{$tpoPlantel->name}}</td>
                    <td>{{$tpoPlantel->usu_alta_id}}</td>
                    <td>{{$tpoPlantel->usu_mod_id}}</td>
                                <td class="text-right">
                                    ...
                                    @endpermission
                                    @permission('tpoPlantels.destroy')
                                    {!! Form::model($tpoPlantel, array('route' => array('tpoPlantels.destroy', $tpoPlantel->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $tpoPlantels->appends(Request::except('page'))->render() !!}
            @else
