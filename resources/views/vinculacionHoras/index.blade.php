...
@section('header')
    ...
    <ol class="breadcrumb">
        ...
        @if ( $query_params = Request::input('q') )
            ...
            <li class="active"><a href="{{ route('vinculacionHoras.index') }}">@yield('vinculacionHorasAppTitle')</a></li>
        ...
        @else
            <li class="active">@yield('vinculacionHorasAppTitle')</li>
        @endif
        -->
        <li class="active">@yield('vinculacionHorasAppTitle')</li>
    </ol>
    ...
    <div class="">
        <h3>
            <i class="glyphicon glyphicon-align-justify"></i> @yield('vinculacionHorasAppTitle')
            @permission('vinculacionHoras.create')
            <a class="btn btn-success pull-right" href="{{ route('vinculacionHoras.create') }}"><i class="glyphicon glyphicon-plus"></i> Crear</a>
            @endpermission
        </h3>
        ...
        <div class="panel panel-default">
            ...
            <div aria-labelledby="headingOne" role="tabpanel" class="panel-collapse collapse" id="collapseOne">
                <div class="panel-body">
                    <form class="VinculacionHora_search" id="search" action="{{ route('vinculacionHoras.index') }}" accept-charset="UTF-8" method="get">
                        ...
                        <div class="form-horizontal">
                            ...
                            -->
                            <div class="form-group">
                                <label class="col-sm-2 control-label" for="q_horas_cont">HORAS</label>
                                <div class=" col-sm-9">
                                    <input class="form-control input-sm" type="search" value="{{ @(Request::input('q')['horas_cont']) ?: '' }}" name="q[horas_cont]" id="q_horas_cont" />
                                </div>
                            </div>
                                                    <!--
                            <div class="form-group">
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            @if($vinculacionHoras->count())
                <table class="table table-condensed table-striped">
                    <thead>
                        <tr>
                            <th>@include('plantillas.getOrderLink', ['column' => 'id', 'title' => 'ID'])</th>
                            <th>@include('CrudDscaffold::getOrderlink', ['column' => 'vinculacions.lugar_practica', 'title' => 'VINCULACION_LUGAR_PRACTICA'])</th>
                        ...
                        <th>@include('CrudDscaffold::getOrderlink', ['column' => 'horas', 'title' => 'HORAS'])</th>
                    ...
                    <tbody>
                        @foreach($vinculacionHoras as $vinculacionHora)
                            <tr>
                                <td><a href="{{ route('vinculacionHoras.show', $vinculacionHora->id) }}">{{$vinculacionHora->id}}</a></td>
                                <td>{{$vinculacionHora->vinculacion->lugar_practica}}</td>
                    <td>{{$vinculacionHora->fec_inicio}}</td>
                    <td>{{$vinculacionHora->fec_fin}}</td>
                    <td>{{$vinculacionHora->horas}}</td>
                    <td>{{$vinculacionHora->usu_alta_id}}</td>
                    <td>{{$vinculacionHora->usu_mod_id}}</td>
                                <td class="text-right">
                                    @permission('vinculacionHoras.edit')
                                    <a class="btn btn-xs btn-primary" href="{{ route('vinculacionHoras.duplicate', $vinculacionHora->id) }}"><i class="glyphicon glyphicon-duplicate"></i> Duplicate</a>
                                    @endpermission
                                    @permission('vinculacionHoras.edit')
                                    <a class="btn btn-xs btn-warning" href="{{ route('vinculacionHoras.edit', $vinculacionHora->id) }}"><i class="glyphicon glyphicon-edit"></i> Editar</a>
                                    @endpermission
                                    @permission('vinculacionHoras.destroy')
                                    {!! Form::model($vinculacionHora, array('route' => array('vinculacionHoras.destroy', $vinculacionHora->id),'method' => 'delete', 'style' => 'display: inline;', 'onsubmit'=> "if(confirm('¿Borrar? ¿Esta seguro?')) { return true } else {return false };")) !!}
                ...
                {!! $vinculacionHoras->appends(Request::except('page'))->render() !!}
            @else
